  ...
  return (
    <div ref={reportRef} className="pb-20 px-3 pt-3 max-w-4xl mx-auto space-y-4">
      ...
      {data.snapshot && (
        <ReportKPIBar
          items={[
            { label: "SKU", value: String(data.snapshot.totalSku), icon: <Package className="w-4 h-4" /> },
            { label: "Выручка", value: fmtRub(data.snapshot.totalRevenue), icon: <DollarSign className="w-4 h-4" />, emphasis: "positive" as const },
            { label: "Валовая прибыль", value: fmtRub(data.snapshot.totalGrossProfit), icon: <TrendingUp className="w-4 h-4" />, emphasis: "positive" as const },
            { label: "Средняя маржа", value: `${data.snapshot.avgMarginPct}%`, icon: <Percent className="w-4 h-4" /> },
            { label: "Активных", value: String(data.snapshot.activeProducts), icon: <Zap className="w-4 h-4" /> },
          ]}
        />
      )}
      ...
      <div className="space-y-1.5">
        {sortedProducts.map((p, i) => (
          <ProductCard
            ...
            product={p}
            index={i}
            expanded={expanded === p.productUuid}
            onToggle={() => setExpanded(expanded === p.productUuid ? null : p.productUuid)}
            onAskAI={() => setChatEntityId(p.productUuid)}
          />
        ))}
      ...
      <AnimatePresence>
        {chatEntityId && (
          <AIChatPanel
            ...
            entityName={data?.products.find(p => p.productUuid === chatEntityId)?.productName}
...
function ProductCard({ product, index, expanded, onToggle, onAskAI }: {
  product: ProductMetrics; index: number; expanded: boolean; onToggle: () => void; onAskAI?: () => void;
}) {
    ...
    >
      ...
      >
        ...
        <div className="w-7 h-7 rounded-full bg-muted flex items-center justify-center text-xs font-bold shrink-0">
          {product.rankEligible ? product.rank : "—"}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-0.5 text-xs text-muted-foreground">
            ...
            {product.revenueChangePct !== null && (
              <span className={`flex items-center gap-0.5 ${product.revenueChangePct >= 0 ? "text-emerald-500" : "text-red-500"}`}>
                {product.revenueChangePct >= 0 ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                {product.revenueChangePct >= 0 ? "+" : ""}{product.revenueChangePct}%
              </span>
            )}
            <span className={`flex items-center gap-0.5 ${tColor}`}>
              {tIcon} {product.marginPct}%
              {product.marginChangePts !== null && (
                <span className={product.marginChangePts >= 0 ? "text-emerald-500" : "text-red-500"}>
                  {product.marginChangePts >= 0 ? "+" : ""}{product.marginChangePts}пп
                </span>
              )}
            </span>
            <span>{product.abcClass}{product.xyzClass}</span>
            {product.categoryName && <span className="text-[10px] opacity-60">{product.categoryName}</span>}
          </div>
        </div>
        ...
        <div className="text-right shrink-0">
          ...
          <div className="text-[10px] text-muted-foreground">
            P{product.marginPercentile}м · P{product.cvPercentile}с · {product.daysInAssortment}дн
          </div>
          <div className="text-[10px] text-muted-foreground">
            {product.basketPenetration}% чеков
          </div>
        </div>
            ...
            <div className="p-3 space-y-4">
                    ...
                    <LineChart data={product.dailyRevenue}>
                      ...
                      <Tooltip
                        ...
                        content={({ active, payload }) => {
                          ...
                          const isAnom = product.anomalyDays.some(a => a.date === d.date);
                          return (
                      ...
                      {/* Anomaly dots */}
                      {product.anomalyDays.map(a => {
                        const dataWithAnomaly = product.dailyRevenue.map(d => ({
                          ...d,
                          isAnomaly: a.date === d.date ? d.value : null,
                        }));
                        return null; // rendered via custom dot below
                      })}
                    </LineChart>
                  </ResponsiveContainer>
                  {product.anomalyDays.length > 0 && (
                    <div className="flex gap-1.5 mt-1 flex-wrap">
                      {product.anomalyDays.slice(0, 3).map(a => (
              ...
              <div className="bg-muted/20 rounded-lg p-3">
                ...
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                  <div
                    className={`h-1.5 rounded-full transition-all ${product.healthScore >= 70 ? "bg-emerald-500" : product.healthScore >= 40 ? "bg-amber-500" : "bg-red-500"}`}
              ...
              {/* Store breakdown */}
              {product.storeBreakdown.length > 0 && (
                <div>
                  <div className="text-xs font-medium text-muted-foreground mb-1">
                    Распределение по магазинам <span className="opacity-60">(HHI: {product.storeHHI})</span>
                  </div>
                  <ResponsiveContainer width="100%" height={Math.max(80, product.storeBreakdown.length * 24)}>
                    <BarChart data={product.storeBreakdown} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                      <XAxis type="number" tick={{ fontSize: 9 }} />
                      <YAxis type="category" dataKey="store" tick={{ fontSize: 9 }} width={80} />
                      <Tooltip formatter={(v: number) => fmtRub(v)} />
                      <Bar dataKey="revenue" radius={[0, 4, 4, 0]}>
                        {product.storeBreakdown.map((_, i) => (
              ...
              {product.crossSell.length > 0 && (
                <div>
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {product.crossSell.map(cs => (
                      <span key={cs.productName} className="text-[10px] px-2 py-0.5 bg-muted rounded-full">
                        {cs.productName.slice(0, 20)} <span className="font-medium">{cs.affinity}%</span>
                      </span>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                ...
                <MetricBadge label="В ассорт." value={`${product.daysInAssortment} дн.`} />
                <MetricBadge label="Класс" value={`${product.abcClass}${product.xyzClass}`} />
                <MetricBadge label="Продаж" value={String(product.netQuantity)} />
                <MetricBadge label="Первая прод." value={product.firstSaleDate || "—"} />
              </div>
...
function exportCSV(products: ProductMetrics[]) {
  const header = "Название;Категория;Выручка;Количество;Маржа%;CV%;Возврат%;HHI;Класс;Сегмент;Тренд;Дней в ассорт.";
  const rows = products.map(p =>
    [p.productName, p.categoryName || "", p.netRevenue, p.netQuantity, p.marginPct, p.cv,
     p.refundRate, p.storeHHI, `${p.abcClass}${p.xyzClass}`, p.segment,
     p.trendDirection, p.daysInAssortment].join(";"),
  );
