...
// ═══════════════════════════════════════════════════════════════════
// AI-задачи для глубокого анализа товаров и магазинов
// ═══════════════════════════════════════════════════════════════════
...
import {
  ...
  storeAnalysisInput,
  storeAnalysisOutput,
} from "./productStoreSchemas";

/**
 * AI-анализ товарного ассортимента.
 * Принимает до 30 товаров с полными метриками, возвращает структурированный разбор.
 */
export const analyzeProductEffectivenessTask = createTask({
  task: `Ты — категорийный менеджер сети вейп-шопов. Проанализируй метрики товаров и выдай структурированный разбор.

Контекст метрик:
- healthScore (0–100) — композитный индекс здоровья товара. >70 = здоров, <40 = проблема.
- marginPct — маржа %. Норма: >25% для A-класса, >15% для B/C.
- cvVsCategory — во сколько раз товар волатильнее своей категории. >1.5 = проблема.
- refundRate + refundRateTrend — % возвратов и тренд. Растущий возврат с refundRate >5% = сигнал проблемы с качеством/описанием.
- storeHHI — концентрация по магазинам (0–10000). >8100 = 90%+ продаж в одной точке = кандидат на расширение.
- basketPenetration — % чеков, в которых встречается товар.
- abcClass — важность товара (A = вклад в 70% выручки, B = следующие 20%, C = остальное).
- xyzClass — стабильность спроса (X = стабильный, Z = непредсказуемый).
- trendDirection + trendCI.significant — тренд и статистическая значимость.
- crossSell — с какими товарами покупают вместе (affinity = % совместных чеков).
- anomalyDays — аномальные дни с выбросами выручки.

Будь конкретным, используй реальные цифры из метрик. Избегай общих фраз вроде "следует улучшить". 
Называй конкретные товары и конкретные цифры. Если данных мало — честно скажи об этом.
Отвечай на русском языке.`,
  inputSchema: productAnalysisInput,
  outputSchema: productAnalysisOutput,
  maxTokens: 4096,
  temperature: 0.5,
});

/**
 * AI-анализ торговых точек.
 * Принимает все магазины с полными метриками, возвращает структурированный разбор.
 */
export const analyzeStoreEffectivenessTask = createTask({
  task: `Ты — операционный директор сети вейп-шопов. Проанализируй метрики торговых точек и выдай структурированный разбор.

Контекст метрик:
- healthScore (0–100) — композитный индекс здоровья точки.
- marginPct — маржа %.
- cvVsNetwork — во сколько раз точка волатильнее сети. >1.5 = проблема.
- rubPerHour — выручка на час работы (₽). Низкое значение = низкая эффективность.
- revenuePerSeller — выручка на продавца.
- churnRate — прокси текучки персонала (%). >30% = высокая.
- lateOpenDays — количество дней с опозданием открытия.
- openingComplianceCorrelation — корреляция опозданий с выручкой. < -0.3 = опоздания статистически значимо связаны с падением выручки.
- lateOpenRevenueImpact — на сколько % ниже выручка в дни опозданий.
- peakHourCoverage — выручка по часам vs среднее по сети. Провал в 18-20 = проблема вечерних часов.
- trendDirection + trendCI.significant — тренд и его значимость.
- clusterLabel — кластер (small/large) для сравнения с похожими точками.
- anomalyDays — аномальные дни.

Будь конкретным, используй реальные цифры. Называй конкретные магазины. 
Если данных по дисциплине открытий недостаточно — честно скажи об этом.
Отвечай на русском языке.`,
  inputSchema: storeAnalysisInput,
  outputSchema: storeAnalysisOutput,
  maxTokens: 4096,
  temperature: 0.5,
});
