...
export function BestShopWidget({ since, until, dateMode: _dm, expanded, onToggle }: Props) {
  ...
  const { data: grossProfit } = useGrossProfit({ since: range.since, until: range.until, enabled: canSeeProfitValue });
  ...
  const rankScore = (shop: typeof rows[0]): number => {
    return canSeeProfitValue ? (marginMap.get(shop.name)?.profit ?? 0) : shop.netRevenue;
  };
  ...
  const detail = (
    ...
    >
      ...
      <div className="rounded-xl p-4 text-white" style={{ backgroundColor: "hsl(var(--chart-2))" }}>
        ...
        <div className={`grid ${canSeeProfitValue ? "grid-cols-4" : "grid-cols-3"} gap-3 text-center`}>
          <div>
      ...
      <div>
        <h4 className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">
          Рейтинг по {canSeeProfitValue ? "прибыли" : "выручке"} ({sorted.length})
        </h4>
        ...
        <div className="flex items-center text-[8px] text-muted-foreground mb-1 px-1 gap-0.5">
          ...
          {canSeeProfitValue && <span className="w-11 text-right">Прибыль</span>}
        </div>
        ...
        <div className="flex items-center gap-0.5 text-[9px] text-muted-foreground mt-1 pt-1 border-t border-border/50 px-1">
          ...
          {canSeeProfitValue && <span className="w-11 text-right tabular-nums">{formatRub(avgProfit)}</span>}
        </div>
      </div>
